...
app = FastAPI(title="Solar Pro Buildings API")
...
if DATABASE_URL:
    ...
    DB_CONFIG = {
        ...
        'password': os.getenv('DB_PASSWORD', 'solar_pro_2024'),
        'database': os.getenv('DB_NAME', 'solar_pro')
    }
    ...
    try:
        ...
        cur.execute("""
            SELECT 
                ...
                COUNT(*) FILTER (WHERE confidence >= 0.5 AND confidence < 0.6) as b_50_60,
                COUNT(*) FILTER (WHERE confidence >= 0.6 AND confidence < 0.7) as b_60_70,
                COUNT(*) FILTER (WHERE confidence >= 0.7 AND confidence < 0.8) as b_70_80,
                COUNT(*) FILTER (WHERE confidence >= 0.8 AND confidence < 0.9) as b_80_90,
                COUNT(*) FILTER (WHERE confidence >= 0.9 AND confidence <= 1.0) as b_90_100,
                COUNT(*) FILTER (WHERE confidence >= 0.5) as c_ge_50,
                COUNT(*) FILTER (WHERE confidence >= 0.6) as c_ge_60,
                COUNT(*) FILTER (WHERE confidence >= 0.7) as c_ge_70,
                COUNT(*) FILTER (WHERE confidence >= 0.8) as c_ge_80,
                COUNT(*) FILTER (WHERE confidence >= 0.9) as c_ge_90,
                COUNT(*) FILTER (WHERE confidence >= 0.8) as split_high,
                COUNT(*) FILTER (WHERE confidence >= 0.7 AND confidence < 0.8) as split_medium,
                COUNT(*) FILTER (WHERE confidence < 0.7) as split_low
            FROM buildings;
        """)
        stats = cur.fetchone()
        ...
        return {
            ...
            "confidence_buckets": [
                {"label": "0.5–0.6", "min": 0.5, "max": 0.6, "count": stats['b_50_60']},
                {"label": "0.6–0.7", "min": 0.6, "max": 0.7, "count": stats['b_60_70']},
                {"label": "0.7–0.8", "min": 0.7, "max": 0.8, "count": stats['b_70_80']},
                {"label": "0.8–0.9", "min": 0.8, "max": 0.9, "count": stats['b_80_90']},
                {"label": "0.9–1.0", "min": 0.9, "max": 1.0, "count": stats['b_90_100']},
            ],
            "cumulative_by_threshold": [
                {"threshold": 0.5, "count": stats['c_ge_50']},
                {"threshold": 0.6, "count": stats['c_ge_60']},
                {"threshold": 0.7, "count": stats['c_ge_70']},
                {"threshold": 0.8, "count": stats['c_ge_80']},
                {"threshold": 0.9, "count": stats['c_ge_90']},
            ],
            "confidence_split": {
                "high": stats['split_high'],
                "medium": stats['split_medium'],
                "low": stats['split_low'],
            },
        }
