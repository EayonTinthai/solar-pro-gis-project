...
/**
 * Payback-period-based color for map polygons.
 * Shorter payback = greener (better investment).
 * @param {number} paybackYears
 * @returns {{ fill: string, stroke: string }}
 */
export function paybackColor(paybackYears) {
  const y = Number(paybackYears);
  if (y <= 3.5) return { fill: '#22c55e', stroke: '#16a34a' }; // Excellent
  if (y <= 4.5) return { fill: '#3b82f6', stroke: '#2563eb' }; // Good
  if (y <= 5.5) return { fill: '#f59e0b', stroke: '#d97706' }; // Moderate
  return { fill: '#ef4444', stroke: '#dc2626' }; // Long payback
}
