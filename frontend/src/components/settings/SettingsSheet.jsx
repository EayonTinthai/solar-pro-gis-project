...
export function SettingsSheet({ open, onOpenChange }) {
  ...
  const planLabel = useMemo(() => {
    if (isFrontendTrialActive) {
      const dateLabel = formatDate(trialExpiresAt);
      return dateLabel ? `Trial (active until ${dateLabel})` : 'Trial (active)';
    }
    if (demoAccessStatus === 'pending') return 'Free (Demo pending)';
    if (demoAccessStatus === 'granted') {
      const dateLabel = formatDate(demoExpiresAt);
      return dateLabel ? `Demo (active until ${dateLabel})` : 'Demo (active)';
    }
    if (demoAccessStatus === 'expired') {
      const dateLabel = formatDate(demoExpiredAt || demoExpiresAt);
      return dateLabel ? `Free (Demo expired on ${dateLabel})` : 'Free (Demo expired)';
    }
    if (accountType === 'demo') {
      const dateLabel = formatDate(demoExpiresAt);
      return dateLabel ? `Demo (active until ${dateLabel})` : 'Demo';
    }
