...
- **NEW in v2.2.0**: Custom polygon analysis for evaluating specific regions
- **NEW in v2.2.0**: Enhanced filtering (area, capacity, payback period, permitting status, accuracy level)
- **NEW in v2.2.0**: Pagination support for large result sets
- **NEW in v2.2.0**: Data quality monitoring endpoints
- **NEW in v2.2.0**: Customizable solar calculation parameters
- **NEW in v2.2.0**: Detailed calculation breakdowns
...
- `GET /buildings/bbox` - Query buildings by bounding box (with advanced filtering)
- `GET /buildings/nearby` - Query buildings by radius (with advanced filtering)

### Solar Analysis

- `POST /solar/calculate` - Calculate solar potential for a building (with custom parameters)
...
- `GET /rankings` - Get top-ranked solar sites based on multi-factor scoring
- `POST /polygon/analyze` - Analyze solar potential for custom geographic areas
- `GET /admin/data-quality` - Get data quality metrics (requires API key)
- `GET /docs/methodology` - Get calculation methodology documentation
